...
abstract class _DetailStore with Store {
  ...
  void listDetailApi(String type) async {
    ...
    switch (type) {
      // 推荐
      case 'recommend':
        response = await MyHttpRequest().dio.get('/api/book_info/recommend',
            queryParameters: {
              "index": params['index'],
              "size": params['size']
            });
      ...
      // 最受欢迎
      case 'popularBooks':
        response = await MyHttpRequest().dio.get('/api/book_info/popular_book?',
            queryParameters: {
              "index": params['index'],
              "size": params['size']
            });
      ...
      // 热门
      case 'hotProduct':
        response = await MyHttpRequest().dio.get('/api/book_info/hot_book',
            queryParameters: {
              "index": params['index'],
              "size": params['size']
            });
